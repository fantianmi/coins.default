  ...
  public String vertify(HttpServletRequest request, HttpServletResponse response) throws IOException { 
    ...
    if ((uusername != null) && (upassword != null)) {
      if (flag) {
		...
		Btc_account_book abook = acounts.getByUidForAcount(vuser.getUid());
		if (abook == null) {
		...
		if (holds.getBtc_holding(vuser.getUid()) != null) {
			...
			Map<Integer,Object> btc_holding_map = holds.getBtc_holdingToMapByUid(vuser
					.getUid());
		...
		Map<Integer, Object> userordermap = orders.getUserSellingOrdersToMapByUid(vuser.getUid());
