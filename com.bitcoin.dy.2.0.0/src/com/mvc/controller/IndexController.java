...
public class IndexController {
	...
	public String usercenter(HttpServletRequest request) throws ParseException {
		...
		Map<Integer, Object> userordermap = orderService.getUserSellingOrdersToMapByUid(user.getUid());
	...
	public String recharge(HttpServletRequest request,
		...
		if (abook == null) {
			...
			BigDecimal ab_cny_show = abook.getAb_cny();
			session.setAttribute("ab_cny", format.trans(ab_cny_show));
		}
		...
		if (banks.getBankByUid(user.getUid()) == null) {
			request.setAttribute("msg", "您未开通银行");
		...
		request.setAttribute("amountToday",
				format.trans(rechargeService.getByUidAmountToday(user.getUid())));
		request.setAttribute("withdrawCNY_limit",
				format.trans(btc_profit.getBtc_profit_withdrawCNY_limit_max()));
		request.setAttribute("withdrawCNY_limit_min",
				format.trans(btc_profit.getBtc_profit_withdrawCNY_limit_min()));
		request.setAttribute("withdrawCNY_poundage",
				format.trans(btc_profit.getBtc_profit_withdrawCNY_poundage()));
		return "withdrawCNY";
	}
	...
	public String tuijie(HttpServletRequest request, HttpServletResponse response) {
		...
		if(us.getByInviteName(user.getUusername())!=null){
			...
			for(int i=0;i<invitelist.size();i++){
				...
				if(awards.get(user.getUid(), ""+iuser.getUid()+"兑换推广赠送")!=null){
					Btc_award award=(Btc_award)awards.get(user.getUid(), ""+iuser.getUid()+"兑换推广赠送").get(0);
					rengouget=award.getCoinamount();
				}
	...
	public String gRegister(HttpServletResponse response,HttpServletRequest request) {
		HttpSession session = request.getSession();
		session.setAttribute("newslist", contents.getNewsLimit());
		if(session.getAttribute("user_amount")==null){
			int user_amount = us.countAllUser();
         ...
         */
         if(request.getParameter("id")!=null){
              ...
              cookieName = cookieHelp.createCookie("tjid", request.getParameter("id"),3600);
	...
	public String factory(HttpServletRequest request) throws ParseException {
		...
		Btc_user user = (Btc_user)session.getAttribute("globaluser");
		Btc_holding yue=holdingService.getBtc_holding(user.getUid(), Integer.parseInt(stockres.getString("stock.factory.stockid")));
		request.setAttribute("stockyue", format.trans(yue.getBtc_stock_amount()));
